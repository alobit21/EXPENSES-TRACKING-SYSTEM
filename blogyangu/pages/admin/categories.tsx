  ...
  const handleCreate = async (data: { name: string; slug: string; description: string }) => {
    try {
      ...
      if (res.ok) {
        ...
        alert(`Category created: ${newCategory.name}`)
      } else {
  ...
  const handleEdit = async (category: Category, data: { name: string; slug: string; description: string }) => {
    try {
      ...
      if (res.ok) {
        ...
        setShowForm(false)
        setEditingCategory(null)
        alert(`Category updated: ${updatedCategory.name}`)
      } else {
        const err = await res.json()
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <header className="mb-8">
          ...
          <p className="text-gray-400">Create, edit, or delete categories for your posts</p>
        </header>
...
/>
          ...
          <p className="text-gray-400 mb-8">You need to be an admin or author to manage categories.</p>
        )}

        {categories.length === 0 ? (
          <div className="text-center py-12">
            <div className="text-gray-400 text-xl">No categories available yet</div>
            <p className="text-gray-400 mt-2">Create a new category to get started!</p>
          </div>
        ) : (
          <div className="grid gap-6">
            {categories.map((cat) => (
              <div
                key={cat.id}
                className="bg-gray-800 rounded-xl shadow-lg p-6 flex justify-between items-start"
              >
                <div>
                  <h3 className="text-lg font-semibold text-gray-200">{cat.name}</h3>
                  <p className="text-sm text-gray-400 mt-1">Slug: {cat.slug}</p>
                  {cat.description && (
                    <p className="text-sm text-gray-300 mt-2 line-clamp-3">{cat.description}</p>
                  )}
                </div>
                {isAdminOrAuthor && (
