...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const byId = !Number.isNaN(maybeId);
  const whereUnique = byId ? { id: maybeId } : { slug: identifier };
  ...
  const existingPost = await prisma.post.findUnique({ where: whereUnique });
  ...
  if (req.method === "PUT") {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      if (files.coverImage) {
        ...
        const uniqueFilename = `post-${existingPost.id}-${timestamp}${fileExtension}`;
