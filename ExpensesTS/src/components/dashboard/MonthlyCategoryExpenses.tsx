...
const MonthlyCategoryExpenses: React.FC = () => {
  const { user } = useAuth();
  const { loading, error, data } = useQuery<MonthlyCategoryExpensesData>(
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-100 dark:bg-gray-900">
        <p className="text-xl text-gray-600 dark:text-gray-300">
          Please log in to view monthly category expenses.
        </p>
      </div>
    );
  }
  ...
  const months = [
    ...new Set(monthlyCategoryExpenses.map((expense) => expense.month)),
  ];

  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 py-8 px-1 sm:px-1 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {months.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-6">
            {months.map((month) => {
              const monthExpenses = monthlyCategoryExpenses.filter(
                ...
                datasets: [
                  {
                    ...
                    backgroundColor: monthExpenses.map(() =>
                      generateRandomColor()
                    ),
                    borderColor: "#ffffff",
                    borderWidth: 2,
